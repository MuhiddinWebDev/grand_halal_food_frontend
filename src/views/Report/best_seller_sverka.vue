    ...
    <div class="p-4">
        <div class="bg-white rounded-lg shadow-md">
            <div class="flex justify-between items-center p-4 bg-gray-100 rounded-t-lg">
                <h2 class="text-xl font-bold">{{ t('most sold sverka') }}</h2>
                    ...
                    <table class="min-w-full bg-white shadow-md rounded-lg overflow-hidden border">
                        ...
                        <tbody>
                            <tr v-for="(item, index) in reportData.data" :key="index"
                                ...
                                <td class="px-4 py-2 border">{{ dayJS(item.order?.datetime).format('DD.MM.YYYY HH:mm')
                                    }}</td>
                                <td class="px-4 py-2 border">{{ item.product?.['title_' + locale] }}</td>
                                <td class="px-4 py-2 text-right border">{{ item.quantity }}</td>
                                <td class="px-4 py-2 text-right border">{{ useSummaFormat(item.price) }}</td>
                                <td class="px-4 py-2 text-right border">
                                    {{ useSummaFormat(item.quantity * item.price) }}
                                </td>
