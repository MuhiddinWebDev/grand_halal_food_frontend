                        ...
                        <n-table :bordered="true" :single-line="false" size="small">
                            ...
                            <tbody>
                                <tr v-for="(item, index) in reportData.data" :key="index" class="report-table-row"
                                    ...
                                    <td>{{ dayJS(item.warranty_date * 1000).format('YYYY-MM-DD HH:mm:ss') }}</td>
                                    <td>
                                        {{ item.stores?.name }} ||
                                    ...
                                    <td>
                                        <div style="cursor: pointer;">
                                            <n-tooltip placement="bottom-start" trigger="hover">
                                                ...
                                                {{ usePhoneFormat(item.clients?.phone) }}
                                            </n-tooltip>
                                        </div>
                                    </td>
                                    <td>{{ item.serial_number }}</td>
                                    <td>
                                        <n-tag :type=" item.status =='approved' ? 'success': item.status == 'unapproved' ? 'warning':'error'">
                                            {{ t(item.status) }}
                                        </n-tag>
                                    </td>
                                </tr>
...
const showReport = () => {
    spinner.value = true;
    reportService.clientBatterySverka(filterHeader.value).then((res) => {
