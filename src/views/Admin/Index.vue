...
const tableColumn = computed(() => [
  ...
  },
  {
    title: t("phone"),
    key: "phone",
    render: (row) => usePhoneFormat(row.phone),
  },
  {
    title: t("role"),
    key: "role",
    render: (row) => globalStore.roleOption.find((r) => r.key === row.role)?.[`name_${locale.value}`],
  },
  {
    title: t("active"),
    key: "status",
...
<template>
  <div class="p-2 space-y-2 bg-white rounded-xl shadow-md overflow-x-auto">
    ...
    <n-modal transform-origin="center" v-model:show="model_act.update">
      <n-card class="w-full max-w-3xl" :bordered="false" role="dialog" aria-modal="true">
        <ModelForm type="update" :id="model_act.update_id" @close="showClose('update')" @update="modalEmit('update')" />
      </n-card>
    </n-modal>
  </div>
</template>